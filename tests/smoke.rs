...
cfg_if! {
    if #[cfg(feature = "rustls")] {
        ...
        fn contexts() -> (async_std_tls::TlsAcceptor, async_std_tls::TlsConnector) {
        ...
        const FRIENDLY_NAME: &'static str = "async-std-tls localhost testing cert";
...
The async-std-tls test suite is about to delete an old copy of one of its
...
description should mention "async-std-tls".
        ...
        fn install_certificate() -> io::Result<CertContext> {
            unsafe {
                ...
                let mut buffer = "async-std-tls test suite".encode_utf16()
                                                         .chain(Some(0))
                                                         .collect::<Vec<_>>();
                ...
                let name = "CN=localhost,O=async-std-tls,OU=async-std-tls,\
                            G=async_std_tls".encode_utf16()
                                          .chain(Some(0))
                                          .collect::<Vec<_>>();
...
The async-std-tls test suite is about to add a certificate to your set of root
and trusted certificates. This certificate should be for the domain "localhost"
with the description related to "async-std-tls". This certificate is only valid
for one day and will be automatically deleted if you re-run the async-std-tls
test suite later.
